...
- To only enable the logical delete operation in the target, "OH Soft Delete" field shall be mapped with the default value "No" in the [Delete Mode](../integrate/mpping-configuration.md#delete-mode) mapping.
...
Below are the known limitations with qTest integration:

* qTest does not support **custom date field** mapping.
* qTest does not support filtering based on custom fields in **criteria** or **lookup queries**. Hence, the usage of only system-defined fields is recommended when forming such queries.
* For hierarchical entities such as **Module**, qTest has limited API support for fetching ancestors. Therefore, if multiple modules share the same name and appear under different hierarchy levels, it is possible that **OpsHub Integration Manager** may pick an unintended match. To avoid this, use the `parentId` and `expand` parameters wherever applicable.
* For **Module** entity lookups, an exact match on the `name` field is used. If multiple results are found, the first match from the response is selected.
* For **lookup on attachment**, qTest does not provide search APIs to retrieve attachments by file name or other properties. Hence, Target LookUp on attachments is not supported.
* When using the **expand=descendants** parameter in module search, the returned hierarchy can become very large. This may result in performance issues for large datasets.
* The **search** parameter in qTest is **case-sensitive**, which can affect matching accuracy unless appropriately handled in the query.
* qTest does not provide reliable support for **soft delete** detection. Once an entity is deleted from qTest, its metadata is no longer accessible through the API.
* Certain fields like **actualStartDate** and **actualEndDate** are updated automatically by qTest based on test execution status and cannot be updated directly through integration.
* Updates to **custom picklist values** in qTest must be manually synchronized across environments to ensure consistent integration behavior.

> Note: For fields not listed in the qTest API documentation, **OpsHub Integration Manager** may not support full CRUD operations depending on API limitations.
...
* Note field of Testlog entity is also used by OIM to store the recovery marker for the entity as the end system is not supporting the custom field configuration for test-log entity. Basically, note field will have both recovery marker information generated by OIM along with actual note content. The first line of the Note field has the recovery marker, and the second line will have the actual note information. Refer the below screenshot for more clarity.  
  ![qTest_Image6_NoteRecovery.png](../assets/qTest_Image6_NoteRecovery.png)  
  ![qTest_Image7_NoteRecovery.png](../assets/qTest_Image7_NoteRecovery.png)

...
Refer to [Grant permissions to Jira user](#grant-permissions-to-jira-user) for step-wise details.
...
In appendix, please refer: [Grant Browse User Global Permission](#grant-browse-user-global-permission) for step-wise details on how to grant this global permission.
...
- To add a user to Jira Software or Service Desk app, refer [Grant access to Jira applications to a User](#grant-access-to-jira-applications-to-a-user)
...
|--------|----------------------------|-------------------|
| **For Version < 5.0** | Read only text | OH_Last_Update |
...
<p align="center">
  <img src="../assets/JiraSystemConfigurationImage_3a.png" width="900px" />
</p>
...
| **Authentication Mode** | Only when Jira is 'Self-managed' | - For authenticating through API Token, select API Token . For authenticating through user password, select Basic Authentication or Cookie-based Authentication.  **Advantage of 'Cookie-based Authentication' over 'Basic Authentication':**  If 'Basic Authentication' is selected, Jira will internally generate a login request on every API call integration sends.  If 'Cookie-based Authentication' is selected, one-time cookie session will be generated, that will be used for following API requests. This reduces load on Jira on the subsequent API calls.  **Note**: From Jira version 10.2.x, refer to [enable cookie based authentication](#Enable-Cookie-Based-Authentication). |
...
| **Plugin fields display name (Beta)** | Always | Consider this input only if you want to add additional third-party plugin fields for mapping. Provide the input as [Comma-Separated Values](#Comma-Separated_Values.28CSV.29). Please note: Read support for these fields having base type as string, number and datetime are supported for now. |
...
For checking whether the field is accessible by the dedicated integration user, refer [Field Helper](#Field-Helper).

* If a field is part of field mapping, then make sure that the field is not hidden from the field configuration associated with the issuetype of that project. For making sure that a field is not hidden, follow the steps in [How to check whether the field is not hidden from field configuration](#How-to-check-whether-the-field-is-not-hidden-from-field-configuration).

* If you want to synchronize the **Epic Link** or **Issue in Epic** links, then you need to add **Epic Link** to the create and update screen. For further details on how to add a field to screen refer [Adding field to Screen](#Adding-field-to-Screen).  
...
* If you want to synchronize the **Parent Link** for the portfolio items, then you need to add **Parent Link** to the create and update screens. For more details on how to add a field to screen, refer [Adding field to Screen](#Adding-field-to-Screen).
...
> **Note** Values for Target_Field_Name, Source_Field_Name, Source_Date_Format, Target_Date_Format will be already available in default mapping of the required field.
...
* {{ spaceName }} will translate and synchronize those entity link references which are matching following text format as per mention sync option configured in mapping.
  ...
  * When Jeditor plugin is enabled on Jira end system to render field type as HTML then {{ spaceName }} will translate those entity link references which are matching text **{jira-server-url}/browse/{project-key}-{entityId}**
    * For example Description is "Sample text <a href="http://jira-local:8080/browse/PROJ1-101"></a>" or Description is "Sample text <a href="https://opshub.atlassian.net/browse/PROJ1-101"></a>"
...
- **Test Entity**  
  Refer to [Target LookUp Configuration](#target-lookup-configuration)
...
- As shown in below image, please specify the interval (in milliseconds) in field **Jira add comment interval**. The maximum acceptable value for this field is 2000 (milliseconds).
  ...
  ![JIRA_add_comment_interval.PNG](../assets/JIRA_add_comment_interval.PNG)
...
- Select boards from the list for which Sprints will be polled and synchronized to the target system. For doing so:
  - Select the **Entity level mandatory settings** ![Entity_advance_configuration_icon_a1.png](../assets/Entity_advance_configuration_icon_a1.png)
...
![IssueConfig](../assets/IssueConfig.png)
![RelationshipMappings](../assets/RelationshipMappings.PNG)
...
![Jira Entity Level Mandatory Settings](../assets/Jira_Entity_Level_Mandatory_Settings1.png)
![JIRA5_1](../assets/JIRA5_1.png)
...
| **Mapped Entity Type**<br>[Link From] | **Entity Type in Relationship Configuration**<br>[Link To] | **Relationship Type**               |
|--------------------------------------|-------------------------------------------------------------|-------------------------------------|
...
* To synchronize Test entity based on a criteria, refer to [Criteria Configuration](#criteria-configuration) section.
...
For Target Lookup Configuration of Zephyr entities, refer to [Target lookup for Zephyr Entities](#target-lookup-for-zephyr-entities) section.
...
* To synchronize custom fields of the Test Execution entity, custom field details should be provided in the JSON input [provided in the system configuration's Metadata details field].
  * Refer to [Find Zephyr Custom Field Id](#find-zephyr-custom-field-id) section for the custom field details.
* Consider the following sample JSON input:
...
- When synchronizing Xray Cloud entities, if default name of any of Xray entity is changed, the same information must be provided in system configuration. For more details, refer to [System Configuration](#system-configuration) section.
...
- The manual test steps can be synced through the `steps` field available on the `{{ spaceName }}` field configurations. Refer to [Mapping limitations](#mapping-limitations) section for reference.
...
> Custom field format:
> `"customFields":[ { "name": "Custom field name", "value": JSON object }, "ID": ID of custom field ]`
...
* If there is any formatting that is present inside HTML `<pre>` tag, then that formatting will be converted into corresponding formatting inside wiki tag `{noformat}`. But, the formatting will not be rendered properly as per wiki `{noformat}` tag behavior, which doesn't allow adding any formatting inside `{noformat}`. Thus, the formatting will be displayed as part of content.
  ...
  **For example -**  
  ...
  But, in Jira, the content will be displayed as "*Test No Format*" as the styling inside `{noformat}` is ignored by default.
...
![JIRA_admin_user.png](../assets/JIRA_admin_user.png)

* On User Management page, click on 'Create User' button. This will display the **Create New User** form  

![JIRA_create_user.png](../assets/JIRA_create_user.png)
...
![JIRA_create_user_form.PNG](../assets/JIRA_create_user_form.PNG)
...
![JIRA_user_manage_group.png](../assets/JIRA_user_manage_group.png)
...
![JIRA_manage_group_form.PNG](../assets/JIRA_manage_group_form.PNG)
...
![JIRA_project_settings.PNG](../assets/JIRA_project_settings.PNG)

* Click on 'Permissions' on the Project Settings page.  

![JIRA_permission_click.png](../assets/JIRA_permission_click.png)

* Click the **Actions** button and select **Edit Permissions**.  

![JIRA_edit_permission.png](../assets/JIRA_edit_permission.png)
...
![JIRA_permission.PNG](../assets/JIRA_permission.PNG)
...
![JIRA_grant_access_user_app.png](../assets/JIRA_grant_access_user_app.png)
...
![JIRA_grant_access_cloud_app.png](../assets/JIRA_grant_access_cloud_app.png)
...
![Jira_whereIsMyField_1A.png](../assets/Jira_whereIsMyField_1A.png)  
![Jira_whereIsMyField2_1A.png](../assets/Jira_whereIsMyField2_1A.png)
...
![jira_project_click.png](../assets/jira_project_click.png)

* Search and select the desired project  
* Click ![JIRA_project_settings_icon.png](../assets/JIRA_project_settings_icon.png)  
* Select **Screens** from the left nav  
* You'll see screen associations for Create, Edit, View operations  
![jira_project_screen_configuration.png](../assets/jira_project_screen_configuration.png)

* Select the screen to edit, scroll to the end and add the field  
![jira_select_field_from_screen.png](../assets/jira_select_field_from_screen.png)
...
![jira_legacy_mode_a1.png](../assets/jira_legacy_mode_a1.png)
...
- Click the **Administration** link on the top bar and select **System**.  
  ![JIRA_admin_system.png](../assets/JIRA_admin_system.png)  
...
- Click **Add Custom Field**.  
  ...
  ![Jira_custom_field_old_a2.png](../assets/Jira_custom_field_old_a2.png)
...
- Under **System Info**, find **Database Type** and **Database URL**.  
  ![JIRA_database_info.PNG](../assets/JIRA_database_info.PNG)
...
- Under **Jira Info**, find the **Version**.  
  ![JIRA_version.png](../assets/JIRA_version.png)
...
3. Edit the project's workflow:  
   ...
   - Click the pencil icon on the workflow  
     ![Jira_Edit_Workflow.png](../assets/Jira_Edit_Workflow.png)  

4. Existing transitions example:  
   ![Jira_Original_Transtions_Jira.png](../assets/Jira_Original_Transtions_Jira.png)  

5. Add restricted transition (e.g., TODO → Completed) for integration user:  
   - Click **Add Transition**  
     ![Jira_ToDo_Completed.png](../assets/Jira_ToDo_Completed.png)  
   - Click **Conditions**  
     ![Jira_Added_Todo_Completed_Transtioa2.png](../assets/Jira_Added_Todo_Completed_Transtioa2.png)  
   - Select **User Is In Group**  
     ![Jira_Add_User_Condtion.png](../assets/Jira_Add_User_Condtion.png)  
   - Choose the `integration-users` group  
     ![Jira_Integration_User_Group.png](../assets/Jira_Integration_User_Group.png)  

6. Transition is now restricted to the integration user only.  
   ![Jira_Any_Any_Transistion_Added.png](../assets/Jira_Any_Any_Transistion_Added.png)  
...
* When no JSON input is provided:
  ...
  * The description must be in this format: **This is the Xray <Entity display name> Issue Type**.  
...
* To find custom field id of Test Step (field of Test entity):

...
![Note](../assets/Note.jpg) In above permissions, Read.\* means there an ACL with \* selected in columns with Read permission is required. Please refer to [Add user access for table](servicenow-quick-connect.md#add-user-access-for-table) for providing Read.\* permission on a table.

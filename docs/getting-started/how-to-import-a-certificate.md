...
* If the file contains a private key (`-----BEGIN RSA PRIVATE KEY-----`) and a certificate (`-----BEGIN CERTIFICATE-----`), refer to the section on [Generate .p12 File](#generate-p12-file) for further steps.

**Option 2: If you only have a chain of certificates (and no private key):**

* In this case, the .pem file will typically contain a series of certificates in the chain, including the server certificate and intermediate certificates, but no private key. Simply follow the import instructions for handling [Importing Certificates](#steps-to-import-certificate-file).

### Generate .p12 File

> **Prerequisite:** You should have OpenSSL installed on your device.

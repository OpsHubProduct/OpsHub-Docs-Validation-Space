...
- Resolved an issue where relationship mapping in OpsHub Integration Manager displayed reversible link types that are not allowed in the end system.  
  ...
  - **Action:** If such invalid link types are mapped in OpsHub Integration Manager, they must be removed according to the steps in the [post-migration checklist](../manage/upgrade/post-migration-checklist.md#update-relationship-mapping-for-jira).
